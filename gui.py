...
class MacroGUI:
    ...
    def build_ui(self):
        ...
        cbo_offset_mode = ttk.Combobox(offset_setting_frame, textvariable=self.offset_mode_var, state="readonly", width=12)
        cbo_offset_mode["values"] = (AppConfig.OFFSET_FIXED, AppConfig.OFFSET_ITER)
        cbo_offset_mode.pack(side=tk.LEFT, padx=3)
        ...
        spin_ox = ttk.Spinbox(offset_setting_frame, from_=-9999, to=9999, textvariable=self.off_x_var, width=6)
        spin_ox.pack(side=tk.LEFT, padx=3)

        ttk.Label(offset_setting_frame, text=" Y偏移：").pack(side=tk.LEFT)
        self.off_y_var = tk.StringVar(value="0")
        spin_oy = ttk.Spinbox(offset_setting_frame, from_=-9999, to=9999, textvariable=self.off_y_var, width=6)
        spin_oy.pack(side=tk.LEFT, padx=3)
    ...
    def _get_play_params(self):
        ...
        return loop_num, wait_sec, speed, off_round, offset_mode, ox, oy, target_set, enable_offset, exclude_set
    ...
    def _run_play_task(self, filepath):
        ...
        if params is None:
            self.root.after(200, lambda: self.set_btn_state(tk.NORMAL))
            return
        loop_num, wait_sec, speed, off_round, offset_mode, ox, oy, target_set, enable_offset, exclude_set = params
